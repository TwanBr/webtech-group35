...
//print_r($postdatam);exit;
    ...
    function submitWallPost($Data = '')
    {
        ...
        $imagename = $this->input->post('userfile');
        $this->load->library('upload', $config);

        $this->upload->do_upload('userfile');
        $data_upload_files = $this->upload->data();
        $image = $data_upload_files['file_name'];
        $result = $this->user->addpost($user_id, $content, $date_created, $imagename);
    ...
    public function invitefriends()
    {
        ...
        if ($fbuser) {
            $userProfile = $facebook->api('/me/user_friends');
            $data['friends'] = $userProfile;
            print_r($data['friends']['data']);
            exit;
            $data['title'] = 'Home';
            $this->load->view("invite-friends.php", $data);
        }
    }
}